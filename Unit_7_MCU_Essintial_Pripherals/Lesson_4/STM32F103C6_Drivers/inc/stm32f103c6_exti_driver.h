...
typedef struct{
		...
		uint16_t 		EXTI_InputLineNumber;	//Specifies the external interrupt GPIO mapping
...
typedef struct{
	...
	uint8_t				IRQ_EN;					//Enable or Disable the EXTI IRQ (that will enable the IRQ MASK in EXTI and also the NVIC interrupt)
												//This parameter must be set based on @ref EXTI_IRQ_define

	void(* p_IRQ_callBack)(void);				//set the C function() which will be called onse the IRQ happened
}EXTI_PinConfig_t;
